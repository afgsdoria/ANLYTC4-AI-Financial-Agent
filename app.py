...
def _load_user_into_session(udata: dict, username: str):
    ...
    st.session_state.report_date     = udata.get("report_date")
...
# ══════════════════════════════════════════════════════════════════════════════
elif st.session_state.app_stage == "onboard":

    st.markdown(
        '<div style="text-align:center;padding:30px 10px 0">'
        '<span class="step-pill"><span class="active-dot"></span>'
        ' &nbsp;Setup Your Financial Profile</span>'
        '</div>',
        unsafe_allow_html=True,
    )

    lpad, main_col, rpad = st.columns([1, 5, 1])
    with main_col:
        st.markdown(
            '<div style="background:linear-gradient(135deg,#0C1A2E,#0F2040);'
            'border:1px solid #1E3A5C;border-radius:20px;padding:32px 40px;'
            'box-shadow:0 12px 40px rgba(0,0,0,.6);margin-bottom:20px">'
            '<h1 style="color:#60A5FA;font-size:1.8rem;font-weight:800;margin:0 0 6px">'
            f'Welcome, {st.session_state.username}!</h1>'
            '<p style="color:#64748B;font-size:.95rem;margin:0">'
            'Complete your profile below — the AI will use this to build your personalized dashboard.</p>'
            '</div>',
            unsafe_allow_html=True,
        )
        ...
        with fi_col3:
            savings_goal_overall = st.number_input(
        ...
        # ── SECTION 3: Your Goal ──────────────────────────────────────────
        st.markdown("#### Describe Your Financial Goal")
        st.markdown(
            '<div style="background:#0C1A2E;border:1px solid #1E3A5C;border-radius:10px;'
            'padding:12px 16px;margin-bottom:10px">'
            '<p style="color:#60A5FA;font-weight:700;margin:0 0 4px;font-size:.85rem">Examples</p>'
            ...
            '"Save for a Japan trip" — AI will infer amount &amp; deadline automatically'
            '</p></div>',
            unsafe_allow_html=True,
        )
        goal_text = st.text_area(
            "Describe your goal (plain language — AI extracts amount & deadline)",
            placeholder="e.g. I want to save for a MacBook Pro by end of the year",
            height=90, key="ob_goal_text",
            label_visibility="collapsed",
        )

        # ── SECTION 4: Upload Previous Tracker (optional) ─────────────────
        st.markdown("#### Upload Previous Financial Tracker")
        st.caption("Optionally upload a CSV, Excel, PDF, or Word file from your old tracker. The AI will use it as extra context.")
        ...
        go_build = st.button("Build My AI Dashboard", use_container_width=True, key="ob_submit")

    if go_build:
        if monthly_income <= 0:
            st.error("⚠️ Please enter your monthly income or allowance to continue.")
            st.stop()
        ...
        # Parse and save goal
        if goal_text.strip():
            parsed = extract_goal_from_text(goal_text, monthly_income)
            save_goal(
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    active_goals = get_active_goals(uname)
    active_goal  = active_goals[0] if active_goals else None

    # Load cached AI analysis if session is empty
        ...
        if st.button(" Save Goal", use_container_width=True):
            if new_goal_txt.strip():
                ...
                st.session_state.sidebar_goal_success = (
                    f"Goal saved & activated!\n\n"
                    f"**{parsed['goal_name']}** — {pesos(parsed['target_amount'])} by {parsed['deadline']}"
                )
                with st.spinner("Updating AI analysis…"):
                    _trigger_ai_analysis()
                st.rerun()
    ...
    (t_dash, t_plan, t_expenses, t_chat, t_settings) = st.tabs([
        "Dashboard", "Action Plan", "Expenses",
        "AI Chat",   "Settings",
    ])
    ...
    with t_dash:
        ...
        if expenses:
            ...
            if ai_categories and ai_amounts and len(ai_categories) == len(ai_amounts):
                ...
                st.caption("🤖 Chart data structured and interpreted by the AI reasoning engine.")
            else:
                df_chart = df_e.groupby("Category", as_index=False)["Amount"].sum()
                st.caption("Chart reflects your logged expense data.")

            ch1, ch2 = st.columns(2)
            with ch1:
                fig_pie = px.pie(
                    df_chart, names="Category", values="Amount", hole=0.42,
            ...
            with ch2:
                df_cat = df_chart.sort_values("Amount", ascending=True)
                fig_bar = px.bar(
    ...
    # ══════════════════════════════════════════════════════════════════════════
    with t_plan:
        st.markdown("## Your AI Action Plan")
        ...
        for ag in all_active:
            ...
            st.markdown(
                ...
                f'<p style="color:#60A5FA;font-weight:700;margin:0 0 6px">🎯 {ag["goal_name"]}</p>'
                f'<p style="color:#94A3B8;font-size:.82rem;margin:0 0 8px">'
                f'{pesos(savings)} saved of {pesos(ag["target_amount"])} · Deadline: {ag["deadline"]}</p>'
                f'</div>',
                unsafe_allow_html=True,
            )
    ...
    with t_chat:
        ...
        st.caption(
            "I'm your personalized finance coach. I answer finance-related questions "
            "and know your income, spending, and goals. Feel free to ask about banks, "
            "investments, loans, or any money topic!"
        )
        ...
        prompt = st.chat_input("Ask about your finances, banks, investments…")
        if prompt:
