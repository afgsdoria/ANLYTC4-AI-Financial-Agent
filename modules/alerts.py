...
# Thresholds as % of monthly income
_THRESHOLDS = {
    "Food":          0.30,
    "Entertainment": 0.15,
    "Shopping":      0.20,
...
def generate_spending_alerts(
    ...
    expenses: list[tuple],          # [(category, amount, date), ...]
) -> list[str]:
    """
    ...
    if total_spending > monthly_income:
        ...
        alerts.append(
            f"🚨 You are spending ₱{over:,.2f} MORE than your income this period!"
        )
    elif spending_ratio > 0.90:
        alerts.append(
            f"⚠️ Spending is at {spending_ratio*100:.0f}% of income — very little room to save."
        )
    elif spending_ratio > 0.80:
        alerts.append(
            f"⚠️ Spending exceeds 80% of income ({spending_ratio*100:.0f}%). "
    ...
    for category, threshold in _THRESHOLDS.items():
        ...
        if cat_total > monthly_income * threshold:
            ...
            alerts.append(
                f"⚠️ {category} spending is {pct:.0f}% of income "
                f"(₱{cat_total:,.2f}). Recommended: ≤{threshold*100:.0f}%."
            )

    # --- Zero savings alert ---
    if total_spending >= monthly_income and monthly_income > 0:
        alerts.append(
            "💡 You have no money left to save this month. "
